...
describe("whitelist-transfer-hook", () => {
  ...
  const [whitelistedAccount] = anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from("whitelisted"), provider.publicKey.toBuffer()],
    program.programId,
  );
  ...
  it("Add user to whitelist", async () => {
    const tx = await program.methods
      ...
      .accountsPartial({
        ...
        whitelistedAccount: whitelistedAccount,
      })
      .rpc();
  ...
  it("Transfer Hook with Extra Account Meta", async () => {
    ...
    transferInstruction.keys.push(
      ...
      { pubkey: whitelistedAccount, isSigner: false, isWritable: false },
